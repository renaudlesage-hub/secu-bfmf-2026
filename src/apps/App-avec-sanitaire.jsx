...
import { ShieldAlert, ClipboardList, Footprints, TriangleAlert, Zap, ChevronRight, Landmark, Droplets, Beer } from "lucide-react";
...
import Stocks from "./apps/stocks.jsx";

// Navigation par hash : chaque app garde sa propre URL partageable.
// #sos et #signaler/... sont les deux seules routes publiques.
const ROUTES = {
  ...
  sanitaire: { titre: "Équipe sanitaire", desc: "Signalements QR (WC, poubelles, lave-mains) + affiches", icon: Droplets, comp: Sanitaire, public: false },
  stocks: { titre: "Stocks bar", desc: "Encodage plaine + étapes, suivi général, alertes stock bas", icon: Beer, comp: Stocks, public: false },
...
function routeDepuisHash() {
  ...
  if (h.startsWith("signaler")) return "signaler"; // supporte #signaler/ID
  return h;
}

export default function App() {
  const [route, setRoute] = useState(routeDepuisHash());

  useEffect(() => {
    const onHash = () => setRoute(routeDepuisHash());
    ...
    <div className="min-h-screen bg-[#11151b] text-slate-100 font-sans flex items-center justify-center p-4">
      ...
      <div className="w-full max-w-md">
        ...
        <div className="text-[10px] text-slate-600 font-mono text-center mt-6 leading-relaxed">
          Config : src/config.js · Apps équipes à diffusion contrôlée<br />
          Routes publiques : #sos et #signaler uniquement
        </div>
      </div>
    </div>
  );
